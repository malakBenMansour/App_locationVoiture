...
public Energie update(@RequestBody Energie e)
{
   return  $energieService.update(e);
    ...
    public ResponseEntity<Energie> updateEnergie(@PathVariable int id, @RequestBody Energie employeeDetails) {
        Energie employee = $energieService.findById(id);
        employee.setNom(employeeDetails.getNom());

        Energie updateBoite = $energieService.update(employee);
